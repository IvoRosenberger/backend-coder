...
class Contenedor {
  ...
  async _readFileOrCreateNewOne() {
    try {
      ...
      error.code === "ENOENT"
        ...
        : console.log(
            `Error Code: ${error.code} | error al leer ${this._filename}`
          );
    }
  }
  ...
  async _createEmptyFile() {
    fs.writeFile(this._filename, "[]", (error) => {
      error
        ? console.log(error)
        : console.log(`File ${this._filename} fue creado`);
    });
  }
  ...
  async getById(id) {
    ...
    try {
      ...
      console.log(
        `Error Code: ${error.code} | error al obterner el ID (${id})`
      );
    }
  }
  ...
  async deleteById(id) {
    try {
      ...
      if (objectIdToBeRemoved) {
        ...
        console.log(`ID ${id} no existe en el file`);
        return null;
      }
    } catch (error) {
      console.log(
        `Error Code: ${error.code} | error al eliminar el ID (${id})`
      );
    }
  }
  ...
  async updateById(id, newData) {
    if(this._validateKeysExist(newData)){
      try {
        ...
        `Error Code: ${error.code} | error al actualizar el ID (${id})`
      }
    } else {
      return false;
    }
  ...
  async addToArrayById(id, objectToAdd) {
    ...
    try {
      ...
      `Error Code: ${error.code} | erorr al actualizar el ID (${id})`
    }
    } else {
      return false;
    }
  ...
  async save(object) {    
    if(this._validateKeysExist(object)) {
      try {
        ...
        console.log(
          `Error Code: ${error.code} | error al guardar en el file`
        );
      }
    } else {
      return false;
  ...
  async deleteAll() {
    try {
      ...
      console.log(
        `error (${error.code}) cuando se elimino`
      );
    }
  }
